...
if ( ! class_exists( 'ECBB_Styles', false ) ) {
	...
	final class ECBB_Styles {
		...
		*/
		...
		/**
		 * Inline meta icons inside composite venue/time/cost rows.
		 *
		 * @param string $scope_sel Scoped part selector.
		 * @return string
		 */
		...
		/**
		 * Leading meta icon selector for a scoped part (row or inline composite).
		 *
		 * @param string $scope_sel        Scoped part selector.
		 * @param string $part_type        Cleaned part slug.
		...
		/**
		 * Parent meta list for a scoped Style 1 / grid meta part (spacing targets the list block).
		 *
		 * @param string $scope_sel Scoped part selector.
		 * @return string
		 */
		...
		*/
		...
		/**
		 * Ordered segments for composite venue/time/cost meta rows.
		 *
		 * @return string[]
		 */
		...
		/**
		 * Parse a combo slug into segment order (e.g. cost_time_venue → venue, time, cost).
		 *
		...
		/**
		 * Build slug from segment order.
		 *
		 * @param string[] $order Segment keys.
		 * @return string
		 */
		...
		/**
		 * @param string[] $segments Unique segments to permute.
		 * @return string[][]
		 */
		...
		/**
		 * @param string[] $segments Segment keys (2 or 3).
		 * @return string[]
		 */
		...
		/**
		 * All venue/time/cost combo permutations for List Style 2.
		 *
		 * @return string[]
		 */
		 ...
		 * @param string $segment venue|time|cost
		 * @return bool
		 */
		...
		/**
		 * @param string[] $slugs Combo slugs.
		 * @return array<string,string>
		 */
		...
		/**
		 * Apply venue/time/cost defaults on a composite meta repeater row.
		 *
		...
		public static function ecbb_part_options_style1() {
			...
			return array_merge( $options, self::ecbb_meta_combo_options_style1() );
		}

		public static function ecbb_part_options_style2() {
			$options = self::ecbb_part_options_shared();

			return array_merge( $options, self::ecbb_meta_combo_options_style2() );
		}
		...
		*/
		...
		/**
		 * Meta parts whose outer list block should receive part margin (Style 1 / grid / Style 2).
		 *
		 * @param array<string,mixed> $ctx
		 * @return bool
		 */
		...
		private static function ecbb_parts_meta_list_unified_rules( array $ctx ) {
			...
			$icon_sel = self::ecbb_resolved_meta_icon_selector(
				$scope_sel,
		...
		private static function ecbb_parts_spacing_rules( array $ctx ) {
			...
			$row_sel         = $meta_list_block ? self::ecbb_meta_row_selector( $scope_sel ) : '';
			$list_sel        = $meta_list_block ? self::ecbb_meta_list_selector( $scope_sel ) : '';
			$rules           = [];
		...
		private static function ecbb_parts_meta_icon_rules( array $ctx ) {
			...
			$icon_sel = self::ecbb_resolved_meta_icon_selector( $scope_sel, $part_type, $list_item_style );
			if ( $icon_sel === '' ) {
				return $rules;
			}
		...
		*/
		...
		/**
		 * Typography rules for layout shell controls (date column, etc.).
		 *
		 * @param array<string,mixed> $settings
		 * @param string              $setting_key
		 * @param string[]            $selectors   Full CSS selectors.
		 * @param callable            $color_fn
		 * @return string[]
		 */
		private static function ecbb_layout_shell_typography_rules( array $settings, $setting_key, array $selectors, callable $color_fn ) {
			...
			foreach ( self::ecbb_breakpoints() as $device => $mq ) {
				$decls = self::ecbb_type_decls( $typo, $device );
				$color_raw = self::ecbb_device_value( $typo['color'] ?? '', $device );
				...
				$decls = self::ecbb_decls_important( $decls );
				if ( $decls === [] ) {
					continue;
				}
				$rules[] = self::ecbb_mq_css_rule( $mq, $selector . '{' . implode( ';', $decls ) . '}' );
			}
		...
		public static function ecbb_layout_shell_css( array $settings, $scope_class, callable $color_fn ) {
			...
			$root       = '.' . $scope_class;
			$card_sel   = $root . ' .event-list-card,' . $root . ' .ecbb-event-card,' . $root . ' .event-grid-card';
			...
			foreach ( self::ecbb_breakpoints() as $device => $mq ) {
				...
				$fg_raw = self::ecbb_device_value( $settings['ecbb_card_text_color'] ?? '', $device );
				...
				if ( $fg !== '' ) {
					$rules[] = self::ecbb_mq_css_rule( $mq, $body_sel . '{color:' . $fg . ' !important;}' );
				}

				if ( class_exists( 'ECBB_Markup', false ) && \ECBB_Markup::ecbb_show_list1_date_column( $settings ) ) {
					$date_bg_raw = self::ecbb_device_value( $settings['ecbb_list1_content_background'] ?? '', $device );
					...
					$divider_raw = self::ecbb_device_value( $settings['ecbb_list1_date_column_border'] ?? '', $device );
					...
					$date_pad_raw = self::ecbb_responsive_spacing( $settings, 'ecbb_list1_date_inner_padding', $device );
					$date_pad     = ! empty( $date_pad_raw ) ? self::ecbb_spacing_css( $date_pad_raw ) : '';
					if ( $date_pad !== '' ) {
						$rules[] = self::ecbb_mq_css_rule(
							$mq,
					...
					$date_margin_raw = self::ecbb_responsive_spacing( $settings, 'ecbb_list1_date_inner_margin', $device );
					$date_margin     = ! empty( $date_margin_raw ) ? self::ecbb_spacing_css( $date_margin_raw ) : '';
					if ( $date_margin !== '' ) {
						$rules[] = self::ecbb_mq_css_rule(
							$mq,
					...
					$date_border = self::ecbb_border_decls( $settings['ecbb_list1_date_inner_border'] ?? [] );
					if ( ! empty( $date_border ) ) {
						$rules[] = self::ecbb_mq_css_rule(
							$mq,
							$root . ' .event-list-card__date-inner{' . implode( ';', self::ecbb_decls_important( $date_border ) ) . '}'
						);
					}
				}
			}
