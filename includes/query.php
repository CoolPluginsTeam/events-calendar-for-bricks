...
if ( ! class_exists( 'ECBB_Query', false ) ) {
	...
	final class ECBB_Query {
		...
		*/
		public static function ecbb_event_time_type( array $settings ) {
			if ( isset( $settings['event_type'] ) && (string) $settings['event_type'] !== '' ) {
				$event_type = (string) $settings['event_type'];
		...
		$legacy_status = isset( $settings['status'] ) ? (string) $settings['status'] : 'all';
		if ( 'past' === $legacy_status ) {
			return 'past';
		}
		if ( 'all' === $legacy_status ) {
			return 'all';
		}
		if ( 'upcoming' === $legacy_status || 'future' === $legacy_status ) {
			return 'upcoming';
		}
		...
		*/
		public static function ecbb_date_meta_query( array $settings ) {
			...
			$time_type    = self::ecbb_event_time_type( $settings );
